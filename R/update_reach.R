...
update_reach <- function(xs_points_path, xs_points, xs_dims, gof_stats,
  ...
  # Determine ReachName
  ReachName <- unique(xs_points_reach$ReachName)
  # Update xs_points data frame
  xs_points <- update_xs_points(ReachName = ReachName, xs_points_reach,
                                xs_points)
  # Update xs_dims data frame
  xs_dims <- update_xs_dims(ReachName = ReachName, xs_points_reach,
                            xs_dims)
  # Update gof_stats data frame
  gof_stats <- update_gof_stats(ReachName, xs_dims, regions,
                                bankfull_elevations)
