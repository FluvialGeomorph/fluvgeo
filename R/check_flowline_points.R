#' @title Check the validity of an `fgm` `flowline_points` data structure
#'
#' @description Checks that the input data structure `flowline_points` meets
#' the requirements for this data structure.
#'
#' @export
#' @param flowline_points   SpatialPointsDataFrame; a `flowline_points` data
...
check_flowline_points <- function(flowline_points) {
  ...
  assert_that(class(flowline_points)[1] == "SpatialPointsDataFrame",
  ...
  assert_that(nchar(unique(flowline_points@data$ReachName[1])) > 0,
  ...
  m_min <- min(flowline_points@data$POINT_M)
  m_max <- max(flowline_points@data$POINT_M)

  ## Calculate min and max z
  m_min_z <- min(flowline_points@data[flowline_points@data$POINT_M == m_min, ]$Z)
  m_max_z <- max(flowline_points@data[flowline_points@data$POINT_M == m_max, ]$Z)
