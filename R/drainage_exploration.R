...
locate_drainage_stream <- function(point) {
  ...
  index <- drainage_index_service(unname(sf::st_coordinates(point)[1, 1:2]))
  index <- drainage_geometry(index, "POINT")
  ...
  line <- drainage_geometry(drainage_feature_service(comid), c("LINESTRING", "MULTILINESTRING"))
...
#'   per layer: layer, status, features, detail), `distance_km`, `retrieved_at`,
...
#' @export
get_drainage_context <- function(location, distance_km = 50) {
  ...
  for (i in seq_along(calls)) {
    ...
    value <- tryCatch(drainage_geometry(calls[[i]](), allowed), error = function(e) {
      detail <<- conditionMessage(e)
      NULL
    })
    layers[i] <- list(value)
    rows[[i]] <- data.frame(layer = key, status = if (is.null(value)) "unavailable" else "available",
      features = if (is.null(value)) 0L else nrow(value), detail = detail)
  }
  list(location = location, layers = layers, status = do.call(rbind, rows),
