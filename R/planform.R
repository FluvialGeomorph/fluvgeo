...
planform <- function(bankline_points) {
  ...
  # Join loop_width to bends1
  bends_length_width <- merge(x = bends_length, y = loop_width, by = "loop",
                           all = TRUE)

  # Remove unneeded fields
  fields <- c("bend_num", "loop", "bend", "bend_POINT_X", "bend_POINT_Y",
              "POINT_X.x", "POINT_Y.x", "bend_radius", "meander_length",
              "meander_width")
  bends_planform <- bends_length_width[, fields]

  # Rename fields
  names(bends_planform)[names(bends_planform) == 'POINT_X.x'] <- 'loop_POINT_X'
  names(bends_planform)[names(bends_planform) == 'POINT_Y.x'] <- 'loop_POINT_Y'
