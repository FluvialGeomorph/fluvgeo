...
check_cross_section <- function(cross_section,
  ...
  if(step %in% c("assign_ids", "watershed_area", "river_position",
    ...
    assert_that(is.data.frame(cross_section@data),
                msg = paste(name, "must be a data frame"))
    assert_that("ReachName" %in% colnames(cross_section@data) &
                  is.character(cross_section@data$ReachName),
                msg = paste("Character field 'ReachName' missing from", name))
    assert_that("Seq" %in% colnames(cross_section@data) &
  ...
  if(step %in% c("watershed_area", "river_position", "station_points",
                 "loop_bend")) {
    assert_that("Watershed_Area_SqMile" %in% colnames(cross_section@data) &
                  is.numeric(cross_section@data$Watershed_Area_SqMile),
                msg = paste("Numeric field 'Watershed_Area_SqMile' missing from",
                            name))

    # Check that all cross sections have watershed areas
    assert_that((sum(is.na(cross_section@data$Watershed_Area_SqMi)) +
                   sum(na.omit(cross_section@data$Watershed_Area_SqMile) == 0)) == 0,
  ...
  if(step %in% c("river_position", "station_points", "loop_bend")) {
    assert_that("POINT_X" %in% colnames(cross_section@data) &
                is.numeric(cross_section@data$POINT_X),
                msg = paste("Numeric field 'POINT_X' missing from", name))
    assert_that("POINT_Y" %in% colnames(cross_section@data) &
                is.numeric(cross_section@data$POINT_Y),
                msg = paste("Numeric field 'POINT_Y' missing from", name))
    assert_that("POINT_M" %in% colnames(cross_section@data) &
                is.numeric(cross_section@data$POINT_M),
                msg = paste("Numeric field 'POINT_M' missing from", name))
    assert_that("Z" %in% colnames(cross_section@data) &
                is.numeric(cross_section@data$Z),
                msg = paste("Numeric field 'Z' missing from", name))
    assert_that("km_to_mouth" %in% colnames(cross_section@data) &
                is.numeric(cross_section@data$km_to_mouth),
    ...
    min_seq <- min(cross_section@data$Seq)
    max_seq <- max(cross_section@data$Seq)

    xs_1 <- cross_section@data[cross_section@data$Seq == min_seq, ]
    min_seq_elevation <- min(xs_1$Z)

    xs_n <- cross_section@data[cross_section@data$Seq == max_seq, ]
  ...
  if(step %in% c("station_points", "loop_bend")) {
    assert_that("from_measure" %in% colnames(cross_section@data) &
                is.numeric(cross_section@data$from_measure),
                msg = paste("Numeric field 'from_measure' missing from", name))
    assert_that("to_measure" %in% colnames(cross_section@data) &
                is.numeric(cross_section@data$to_measure),
                msg = paste("Numeric field 'to_measure' missing from", name))
  }

  # Step: loop_bend
  if(step %in% c("loop_bend")) {
  assert_that("loop" %in% colnames(cross_section@data) &
                is.numeric(cross_section@data$loop),
              msg = paste("Numeric field 'loop' missing from", name))
  assert_that("bend" %in% colnames(cross_section@data) &
                is.numeric(cross_section@data$bend),
