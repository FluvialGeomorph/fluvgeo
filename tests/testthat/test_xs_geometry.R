...
test_that("Check that fields exist by name", {
  ...
  expect_true("ground_elev" %in% colnames(xs_geometry(sin_xs_points_4, 103.0)))
})
...
test_that("Check that fields are of the correct data type", {
  ...
  expect_true(is.numeric(xs_geometry(sin_xs_points_4, 103.0)$ground_elev))
})

test_that("Check dimensions from known stream", {
  expect_equal(xs_geometry(xs_points = sin_xs_points_4,
                           detrend_elevation = 103.5)$xs_width,
               104.0971, tolerance = 1e-2)
  expect_equal(xs_geometry(xs_points = sin_xs_points_4,
                           detrend_elevation = 103.5)$xs_depth,
               3.4677,   tolerance = 1e-2)
  expect_equal(xs_geometry(xs_points = sin_xs_points_4,
                           detrend_elevation = 103.5)$xs_area,
               274.3170, tolerance = 1e-2)
  expect_equal(xs_geometry(xs_points = sin_xs_points_4,
                           detrend_elevation = 103.5)$ground_elev,
